...
public class LdaModel {
	...
	public void initializeModel(Documents docSet) {
		...
		for(int m = 0; m < M; m++){
			...
			int N = docSet.docs.get(m).docWords.length; // N是一个文档的单词个数，不考虑是否重复
			doc[m] = new int[N];
			for(int n = 0; n < N; n++){
	...
	public void inferenceModel(Documents docSet) throws IOException {
		// TODO Auto-generated method stub
		if(iterations < saveStep + beginSaveIters){// 训练前检测参数大小
			System.err.println("Error: the number of iterations should be larger than " + (saveStep + beginSaveIters));
			System.exit(0);
		}
		for(int i = 0; i < iterations; i++){
			System.out.println("Iteration " + i);
			//beginSaveIters=80,saveStep = 10
			if((i - beginSaveIters >= 0) && (((i - beginSaveIters) % saveStep) == 0)){
				//Saving the model
				System.out.println("Saving model at iteration " + i +" ... ");
				//Firstly update parameters
				updateEstimatedParameters();
				//Secondly save model variables
	...
	private int sampleTopicZ(int m, int n) {
		...
		int oldTopic = z[m][n];
		nmk[m][oldTopic]--; //number of words in doc m assigned to topic 
		nkt[oldTopic][doc[m][n]]--; //number of term assigned to topic
		nmkSum[m]--;   //total number of words in document m
		nktSum[oldTopic]--; //total number of words assigned to topic
